...
function tsviewer_activate() {
    ...
    if($db->num_rows($group_exists) > 0) {
        $gid = $db->fetch_field($group_exists, "gid");
    } else {
        $gid = $db->insert_query("settinggroups", array(
            'name' => 'tsviewer_group',
            'title' => 'TeamSpeak Viewer Settings',
            'description' => 'Settings for the TeamSpeak 6 Global Viewer.',
            'disporder' => 5,
            'isdefault' => 0
        ));
    }
...
function tsviewer_run() {
    ...
    if (file_exists($cache_file) && (time() - filemtime($cache_file) < $cache_time)) {
        ...
        $decrypted_data = tsviewer_decrypt($raw_data, $api_key);
        $cache_data = json_decode($decrypted_data, true);
    ...
    $client_url = $base_api . '/clientlist?-times&-away';
    $channel_url = $base_api . '/channellist';
    $server_url = $base_api . '/serverinfo';

    $channels_data = ts_fetch_api($channel_url, $api_key);
    $clients_data = ts_fetch_api($client_url, $api_key);
    $server_data = ts_fetch_api($server_url, $api_key);
    ...
    if ($clients_data) {
        ...
        if (isset($clients_data['body'])) {
            foreach ($clients_data['body'] as $client) {
                if (isset($client['client_type']) && $client['client_type'] == 0 && !empty($client['client_nickname'])) {
                    ...
                    $idle_ms = isset($client['client_idle_time']) ? (int)$client['client_idle_time'] : 0;
                    $idle_sec = floor($idle_ms / 1000);
                    $h = floor($idle_sec / 3600);
                    $m = floor(($idle_sec % 3600) / 60);
                    $s = $idle_sec % 60;
                    $idle_str = ($h > 0 ? "{$h}h " : "") . ($m > 0 ? "{$m}m " : "") . "{$s}s";
        ...
        $payload = json_encode(['count' => $ts_count, 'users' => $ts_online_users, 'status' => $ts_status, 'last_seen' => time()]);
        ...
        if (file_exists($cache_file)) {
            ...
            if ($cache_data) {
                ...
                $diff = time() - $last_seen;
                $ago = ($diff >= 3600) ? floor($diff / 3600) . " hr ago" : (($diff >= 60) ? floor($diff / 60) . " min ago" : "just now");
                $ts_status = str_replace('{last_seen}', $ago, $tpl_offline);
            }
        }
    }
}
...
function tsviewer_admin_preview() {
    ...
    if (isset($mybb->input['module']) && $mybb->input['module'] == 'config-settings' && isset($mybb->input['action']) && $mybb->input['action'] == 'change') {
        ...
        if ($db->fetch_field($query, "name") == 'tsviewer_group') {
            ...
            <script type="text/javascript">
                function updateTSPreview() {
                    ...
                    const treePreview = "<div style=\'margin-bottom:8px;\'>"+chHtml.replace("{channel_name}", "Lobby")+"</div><div>"+usrHtml.replace("{username}", "User1").replace("{away_status}", "").replace("{idle_color}", "#2ecc71").replace("{idle_time}", "02m")+"</div>";
